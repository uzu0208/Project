...
public class LoginController {
    ...
    @PostMapping("checkPasswordToDelete/login")
    public String checkPasswordToDelete(HttpServletRequest request, HttpServletResponse response, HttpSession session,
        ...
        if (service.checkPassword(checkdto.getUserid(), password)){
            ...
            return "redirect:/deleteMember/login";
        }else {
            response.setContentType("text/html; charset=UTF-8");
            PrintWriter out = response.getWriter();
            out.println("<script>alert('비밀번호가 맞지 않습니다.'); history.go(-1) </script>");
