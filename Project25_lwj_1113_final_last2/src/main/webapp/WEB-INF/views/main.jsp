  ...
  <head>
	...
	$(document).ready(function() {
			...
			$.ajax({
	               ...
	               success:function(data, status, xhr){
	            	   ...
	            	   $("#pop_info_"+index+" #commenttable").empty();
	            		   ...
	            		   $("#pop_info_"+index+" #commenttable").append("<tr><td>"+rname+"</td><td>"+comments+"</td><td>"+writedate+"</td><td><button id='deletebtn"+i+"' class='btn deletebtn btn-danger' data-num='"+rid+"' data-no='"+rno+"'>삭제</button></td></tr>");
	            	   }
	 	   			 $("#pop_info_"+index+" #commenttable").append("<tr><td colspan='3'>");
	 	   			   for(i=0;i<c_PageNum;i++){ // append로 테이블의 뒷부분에 넣어짐 == 페이징부분 
	 	   				if(i!=c_curPage){$("#pop_info_"+index+" #commenttable").append("<a data-num='"+(parseInt(i)+1)+"' title='"+index+"' class='c_pagebutton' href='#pop_info_"+index+"'> "+parseInt(i+1))+"</a>";}
	 	   					...
	 	   					$("#pop_info_"+index+" #commenttable").prepend("<tr><th>유저이름</th><th>작성내용</th><th>게시시간</th><th>삭제</th></tr>");
		...
		$(".btn_close").on("click", function() {
			...
			$("#pop_info_"+index+" .commentContainer").attr("style", "display: none");
		});
		               ...
		               success:function(data, status, xhr){
		            	   ...
		            	   for(i=0;i<c_perPage;i++){
		            		   ...
		            		   if(tbody!=null){ // 만약 페이지가 5페이지인데 나올 갯수가 14개면 (perpage=3) 3번쨰 실행시에 데이터가 없어서 페이지가 안나온다 == 중간에 에러가 있으면 아래 코드가 실행되지 않음 그래서 조건처리한 것.
			            		   ...
			            		   let writedate = (tbody['writedate']);
			            		   let rno = (tbody['rno']);
		            		   $("#pop_info_"+index+" #commenttable").append("<tr><td>"+rname+"</td><td>"+comments+"</td><td>"+writedate+"</td><td><button id='deletebtn"+i+"' class='btn deletebtn btn-danger' data-no='"+rno+"' data-num='"+rid+"' >삭제</button></td></tr>");
		            		   }
		            	   }
		            	   $("#pop_info_"+index+" #commenttable").append("<tr><td colspan='3'>");
		 	   			   for(i=1;i<c_PageNum+1;i++){
		 	   				if(i!=c_curPage){$("#pop_info_"+index+" #commenttable").append("<a data-num='"+i+"' title='"+index+"' class='c_pagebutton' href='#pop_info_"+index+"'> "+parseInt(i))+"</a>";}
		 	   					...
		 	   					$("#pop_info_"+index+" #commenttable").append("</td></tr>");
		 	   				$("#pop_info_"+index+" #commenttable").prepend("<tr><th>유저이름</th><th>작성내용</th><th>게시시간</th><th></th></tr>");
		...
		$(".btn_comment").on("click", function(){
			 var index = $(this).attr("title");
			 $("#pop_info_"+index+" .commentContainer").attr("style", "display: block");
			 $("#pop_info_"+index).animate({"left": "-500px"}, "slow");
			 $("#pop_info_"+index+" .commentContainer").animate({"right": "150px"}, "slow");
		});
	...
	#centerAddr {display:block;margin-top:2px;font-weight: normal; line-height: 15px;}
	...
	<jsp:include page="choosemap/choosemap.jsp" flush="true" />
	  ...
	  <div class="col-lg-4 col-md-12" id="mainTableContainer">		
		...
		<tbody>
			...
			<% index++;%> <!-- pagedto의 list(MapDTO)를 반복할때 증가할 수 있게 선언 -->
					<td colspan="3">
						...
						<div id="pop_info_<%=index%>" class="pop_wrap" style="display: none;" >
							...
							<div class="map_wrap" title="<%=index%>" height="250px">
							...
							<table class="table" id="popuptable">
								<tr><td></td></tr>
								<tr><td>
									<c:choose> <c:when test="${list.rdnmadr!=null}"><p class="dsc2">위치 : ${list.rdnmadr}</p> </c:when>
									<c:otherwise><p class="dsc2">위치 : ${list.lnmadr}</p></c:otherwise>
									</c:choose>		
								</td></tr>
								
								<tr><td>
									<div class="dsc3 input-group">
									<span class="input-group-text">축제 설명</span>
									<textarea class="form-control" aria-label="With textarea">${list.fstvlCo}</textarea></div>
								</td></tr>
								
								<tr><td>
								<p class="dsc4">전화번호 : ${list.phoneNumber}</p>
								</td></tr>
								
								<tr><td><!-- 홈페이지 url이 www로 시작할경우 http를 붙여줘서 제대로 하이퍼링크 이동하도록 -->
								<c:if test="${(list.homepageUrl).substring(0,4)=='http'}"><p class="dsc5">홈페이지 : <a id="clickdetail" href="${list.homepageUrl}" target="_blank">${list.homepageUrl}</a></p></c:if>
								<c:if test="${(list.homepageUrl).substring(0,4)!='http'}"><p class="dsc5">홈페이지 : <a id="clickdetail" href="https://${list.homepageUrl}" target="_blank">${list.homepageUrl}</a></p></c:if>
		 						</td></tr>
		 						</table>
								    ...
								    <div class="col col-sm-4">
            								<button type="button" title="<%=index%>" class="btn btn_comment btn-outline-dark ">댓글보기</button>
            								<button type="button" title="<%=index%>" class="btn btn_close btn-outline-dark">닫기</button>
								    </div>
								...
								<div class="container-fluid commentContainer" style="display: none;">
								...
								<div class="container buttoncontainer"  >
							        <div class="col-auto input-group">
							        	<textarea rows="" cols="" name="comments" class="btn_submit_text form-control" ></textarea><!-- 글쓴 내용 -->	
							        </div>
								    ...
								    <div class="col col-sm-8"></div>
								    <div class="col col-sm-4">
            								<input type="submit" title="<%=index%>" class="btn btn_submit btn-primary" value="댓글작성"   />
									<button type="button" title="<%=index%>" class="btn commentCloseBtn btn-secondary"   >닫기</button>
								    </div>
			...
			<%-- pagedto가 null일 때 계산 시 오류 발생 방지 --%>
			<c:choose>
			...
			<c:if test="${endPage > pageNum}">
				...
				<tr>
					<td colspan="3">
							...
							<ul class="pagination justify-content-center">
								...
								<li class="page-item">
									<a class="page-link" aria-label="Previous"
									   href="main?curPage=${startPage - perPage}&city=${city}&province=${province}&citykey=${province}&SearchMode=${pagedto.location.SearchMode}">
										이전
									</a>
								</li>
								</c:when>
						...
						<p class="text-md-center">전체 <fmt:formatNumber maxFractionDigits="0" value="${pageNum}"/>페이지가 검색되었습니다.</p>
					</td>
				</tr>
